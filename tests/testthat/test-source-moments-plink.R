...
test_that("source_moments_plink matches the in-memory source moments", {
  X <- matrix(
    c(
      0, 1, 2,
      1, 1, NA,
  ...
  streamed <- source_moments_plink(
    file,
    block_size = 2L,
    n_threads = 1L
  )
  direct <- source_moments(X, center = FALSE, n_threads = 1L)

  expect_s3_class(streamed, "ldtl_source_moments")
  ...
  expect_equal(streamed$R_source, direct$R_source, tolerance = 1e-13)
  ...
  expect_equal(diag(streamed$R_source), rep(1, 3), tolerance = 1e-13)
  expect_equal(streamed$passes, 2)
  expect_lt(streamed$working_bytes, 1000)

  cached <- source_moments_plink(
    file,
    R_source = direct$R_source,
    block_size = 2L,
    n_threads = 1L
  )
  expect_false(cached$covariance_computed)
  expect_equal(cached$R_source, direct$R_source, tolerance = 1e-14)
  ...
  expect_error(
    source_moments_plink(file, R_source = diag(2)),
...
test_that("source_moments_plink validates PLINK raw rows", {
  file <- tempfile(fileext = ".raw")
  on.exit(unlink(file))
  writeLines(c(
  ...
  expect_error(source_moments_plink(file), "too few dosage fields")
  expect_error(source_moments_plink(file, block_size = 0), "positive integer")
  expect_error(source_moments_plink(file, n_threads = -1), "zero or a positive")
})

test_that("source_moments_plink rejects a monomorphic variant", {
  X <- cbind(c(0, 1, 2), c(1, 1, 1))
  file <- write_plink_raw_fixture(X, c("v1", "v2"))
  on.exit(unlink(file))

  expect_error(source_moments_plink(file), "nonpositive variance")
})
